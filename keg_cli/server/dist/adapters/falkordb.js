...
class FalkorDBAdapter {
    ...
    graph = null;
    graphName;
    constructor(host = 'localhost', port = 6379, graphName = DEFAULT_GRAPH) {
        this.host = host;
        this.port = port;
    ...
    async connect() {
        let FalkorDB;
        try {
            FalkorDB = await Promise.resolve().then(() => __importStar(require('falkordb')));
        }
        catch (_) {
            throw new Error('falkordb package not found. Install: npm install falkordb\n' +
    ...
    async query(cypher, _parameters = {}) {
        if (!this.graph)
        ...
        const rows = (result.data ?? []).map(rowArray => {
            ...
            columns.forEach((col, idx) => {
                row[col] = this.normalizeValue(rowArray[idx]);
            });
            return row;
        });
        return { columns, rows };
    ...
    normalizeValue(value) {
        ...
        if (typeof value === 'object' && value !== null) {
            ...
            if ('labels' in v && 'id' in v && 'properties' in v) {
                ...
                const props = this.normalizeProperties(v.properties);
                return {
                    __type: 'node',
                    id: String(v.id),
                    label: labels[0] ?? 'Node',
            ...
            if ('nodes' in v && 'edges' in v) {
                return {
                    __type: 'path',
                    nodes: v.nodes.map(n => this.normalizeValue(n)),
                    edges: v.edges.map(e => this.normalizeValue(e)),
                };
            }
            const result = {};
    ...
    normalizeProperties(props) {
        if (!props)
            return {};
        return Object.fromEntries(Object.entries(props).map(([k, v]) => [k, this.normalizeValue(v)]));
    }
    async getSchema(options = {}) {
        ...
        const nodeTypesFull = await Promise.all(nodeLabels.map(async (label) => {
            const propResult = await this.query(`MATCH (n:${label}) RETURN keys(n) AS props LIMIT 100`, {});
            const allKeys = new Set();
            for (const row of propResult.rows) {
                for (const key of (row.props ?? []))
                    allKeys.add(key);
            }
            const properties = {};
        ...
        const relTypesFull = await Promise.all(relTypes.map(async (type) => {
            const propResult = await this.query(`MATCH ()-[r:${type}]->() RETURN keys(r) AS props LIMIT 100`, {});
            const allKeys = new Set();
            for (const row of propResult.rows) {
                for (const key of (row.props ?? []))
                    allKeys.add(key);
            ...
            for (const key of allKeys)
                properties[key] = 'ANY';
            // Get FROM/TO types
            const connResult = await this.query(`MATCH (a)-[:${type}]->(b) RETURN DISTINCT labels(a)[0] AS from_label, labels(b)[0] AS to_label LIMIT 5`, {});
            return {
                name: type,
                fromType: connResult.rows[0]?.from_label ?? '?',
                toType: connResult.rows[0]?.to_label ?? '?',
                properties,
            };
        }));
...
//# sourceMappingURL=falkordb.js.map