...
const viz_server_js_1 = require("./viz-server.js");
const kuzu_js_1 = require("./adapters/kuzu.js");
const neo4j_js_1 = require("./adapters/neo4j.js");
const falkordb_js_1 = require("./adapters/falkordb.js");
// ═══════════════════════════════════════════════════
// CONNECTION REGISTRY
...
async function getOrCreateDefaultConnection() {
    ...
    switch (defaultDb) {
        case 'kuzu':
            adapter = new kuzu_js_1.KuzuAdapter(process.env.KEG_KUZU_PATH ?? './keg_data');
            break;
        case 'neo4j':
            adapter = new neo4j_js_1.Neo4jAdapter(process.env.KEG_NEO4J_URI ?? 'bolt://localhost:7687', process.env.KEG_NEO4J_USER ?? 'neo4j', process.env.KEG_NEO4J_PASSWORD ?? '');
            break;
        case 'falkordb':
            adapter = new falkordb_js_1.FalkorDBAdapter(process.env.KEG_FALKORDB_HOST ?? 'localhost', parseInt(process.env.KEG_FALKORDB_PORT ?? '6379', 10));
            break;
        default:
...
async function resolveConnection(db) {
    const key = db ?? defaultDb;
    if (connections.has(key))
        return connections.get(key);
    if (!db)
        return getOrCreateDefaultConnection();
...
// ═══════════════════════════════════════════════════
async function handleKgConnect(args) {
    const { db_type, connection_string, alias, username, password, set_default = true } = args;
    const key = alias ?? db_type;
    let adapter;
    switch (db_type) {
        case 'kuzu':
            adapter = new kuzu_js_1.KuzuAdapter(connection_string ?? process.env.KEG_KUZU_PATH ?? './keg_data');
            break;
        case 'neo4j':
            adapter = new neo4j_js_1.Neo4jAdapter(connection_string ?? process.env.KEG_NEO4J_URI ?? 'bolt://localhost:7687', username ?? process.env.KEG_NEO4J_USER ?? 'neo4j', password ?? process.env.KEG_NEO4J_PASSWORD ?? '');
            break;
        case 'falkordb':
            const parts = (connection_string ?? '').replace('redis://', '').split(':');
            adapter = new falkordb_js_1.FalkorDBAdapter(parts[0] ?? process.env.KEG_FALKORDB_HOST ?? 'localhost', parseInt(parts[1] ?? process.env.KEG_FALKORDB_PORT ?? '6379', 10));
            break;
    }
    await adapter.connect();
...
async function handleKgQuery(args) {
    const { query, db, parameters = {}, limit = 10000, timeout_ms = 30000 } = args;
...
async function handleKgExplore(args) {
    const { target, hops = 2, db, edge_types, filters = {}, direction = 'both', limit = 5000, include_metrics = false, } = args;
    const adapter = await resolveConnection(db);
    ...
    if (include_metrics && payload.nodes.length > 0) {
        ...
        for (const node of payload.nodes) {
            node.properties['_degree'] = degreeMap.get(node.id) ?? 0;
        }
    }
    return {
        content: [{
...
async function handleKgSchema(args) {
    const { db, sample_data = true, include_counts = true } = args;
    const adapter = await resolveConnection(db);
    const schema = await adapter.getSchema({ sampleData: sample_data, includeCounts: include_counts });
    return { content: [{ type: 'text', text: JSON.stringify(schema, null, 2) }] };
}
async function handleKgMetrics(args) {
    const { db, deep = false } = args;
    const adapter = await resolveConnection(db);
  ...
  `;
    ...
    const totalNodes = nodeCounts.rows.reduce((sum, r) => sum + r.count, 0);
    const totalEdges = edgeCounts.rows.reduce((sum, r) => sum + r.count, 0);
    ...
    const metrics = {
        ...
        nodes_by_type: Object.fromEntries(nodeCounts.rows.map((r) => [r.label, r.count])),
        edges_by_type: Object.fromEntries(edgeCounts.rows.map((r) => [r.type, r.count])),
    };
    if (deep) {
        // Triangle count (expensive but Kuzu handles it well with WCO joins)
        try {
            const triResult = await adapter.query(`MATCH (a)-[:KNOWS]->(b)-[:KNOWS]->(c)-[:KNOWS]->(a) RETURN COUNT(*) AS triangles`, {});
...
async function handleKgVizStart(args) {
    const { port = VIZ_PORT, auto_open = true } = args;
    ...
    vizServer = new viz_server_js_1.VizServer({ port, pluginRoot: PLUGIN_ROOT, autoOpen: auto_open });
...
async function handleKgVizStop(_args) {
    if (!vizServer?.isRunning()) {
...
async function handleKgVizStatus(_args) {
    if (!vizServer?.isRunning()) {
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'not_running' }) }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify(vizServer.status()) }] };
}
async function handleKgVisualizePush(args) {
    const { nodes = [], edges = [], layout = 'force', merge = true, title, highlight_ids, center_on } = args;
    if (!vizServer?.isRunning()) {
        // Auto-start if not running
        vizServer = new viz_server_js_1.VizServer({ port: VIZ_PORT, pluginRoot: PLUGIN_ROOT, autoOpen: true });
        await vizServer.start();
    }
...
async function handleKgCommunityDetect(args) {
    const { db, node_label, rel_type, max_iterations = 10 } = args;
    const adapter = await resolveConnection(db);
...
async function handleKgExplain(args) {
    const { query, db } = args;
    const adapter = await resolveConnection(db);
    try {
        ...
        return { content: [{ type: 'text', text: `EXPLAIN not supported by this adapter: ${e.message}` }] };
    }
}
async function handleKgShellExec(args) {
    const { action, bash_command, script_path, query, output_path, output_format = 'csv' } = args;
    ...
    switch (action) {
        ...
        case 'run_script': {
            if (!script_path)
                throw new Error('script_path required for run_script');
            const content = fs.readFileSync(script_path, 'utf-8');
            const statements = content
                .split(/;[\s]*\n/)
                .map(s => s.trim())
                .filter(s => s && !s.startsWith('--'));
            const adapter = await resolveConnection(args.db);
            const results = [];
            for (const stmt of statements) {
                try {
                    ...
                    results.push({ statement: stmt.slice(0, 80) + '...', error: e.message, status: 'error' });
                }
            }
        ...
        case 'export': {
            if (!query || !output_path)
                throw new Error('query and output_path required for export');
            const adapter = await resolveConnection(args.db);
            ...
            if (output_format === 'json') {
                ...
                const lines = [cols.join(','), ...result.rows.map((r) => cols.map(c => JSON.stringify(r[c] ?? '')).join(','))];
        ...
        case 'raw_bash': {
            if (!bash_command)
                throw new Error('bash_command required for raw_bash');
            try {
...
// ═══════════════════════════════════════════════════
const server = new index_js_1.Server({ name: 'keg-grokker', version: '1.0.0' }, { capabilities: { tools: {} } });
